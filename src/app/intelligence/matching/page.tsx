...
import { architects, brands, products, projects } from "@/lib/mock-data";
import {
  productMomentumData,
...
function computeBriefProductMatches(brief: Brief) {
...
function computeBriefArchitectMatches(brief: Brief) {
...
function computeProjectProductMatches() {
...
function computeRoomMatches() {
  ...
  return Object.entries(roomData).map(([roomName, data]) => {
    const topCategories = [...data.categories.entries()].sort((a, b) => b[1] - a[1]);
...
function computeRecommendationLogic() {
  const signals = [
    {
      name: "Architect Influence",
      weight: 25,
      description: "How strongly an architect's usage patterns influence product recommendations",
      factors: ["Product saves", "Spec downloads", "Board additions", "Enquiries generated"],
    },
    {
      name: "Product Momentum",
      weight: 20,
      description: "Current trending strength and growth trajectory of products",
      factors: ["View growth rate", "Save velocity", "Board additions", "Spec inclusions"],
    },
    {
      name: "Category Fit",
      weight: 20,
      description: "Whether the product fills a missing category gap in the brief or project",
      factors: ["Room requirements", "Missing categories", "Category coverage"],
    },
    {
      name: "Style Alignment",
      weight: 15,
      description: "How well the product matches the stated style preferences and materials",
      factors: ["Style tag overlap", "Material keywords", "Design language"],
    },
    {
      name: "Co-occurrence",
      weight: 10,
      description: "Products frequently used together in projects, boards, and specifications",
      factors: ["Project co-usage", "Board co-existence", "Spec room pairings"],
    },
    {
      name: "Brand Affinity",
      weight: 10,
      description: "Whether the brand is already used or preferred by the architect",
      factors: ["Existing brand usage", "Brand influence score", "Supplier relationship"],
    },
  ];
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-semibold ${
        active ? "bg-emerald-light text-emerald" : "bg-surface text-muted"
      }`}
    >
      {active && <Check size={8} />}
      {label}
    ...
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full">
      ...
      {stylePositions.map((sp) =>
        catPositions.map((cp) => {
          ...
          return (
            <path
              key={`${sp.label}-${cp.label}`}
              d={`M ${leftX + 60} ${sp.y} C ${leftX + 180} ${sp.y}, ${rightX - 180} ${cp.y}, ${rightX - 60} ${cp.y}`}
              fill="none"
              stroke="#0a0a0a"
              strokeWidth={strokeW}
              opacity={opacity}
      ...
      {stylePositions.map((sp) => (
        <g
          key={sp.label}
          onMouseEnter={() => setHoveredStyle(sp.label)}
        ...
        >
          ...
          <text x={leftX} y={sp.y + 4} textAnchor="middle" className="text-[10px] font-semibold" fill={hoveredStyle === sp.label ? "#fff" : "#0a0a0a"}>
            {sp.label}
          </text>
        </g>
      ))}
      ...
      {catPositions.map((cp) => {
        ...
        return (
          <g key={cp.label}>
            ...
            <text x={rightX} y={cp.y + 4} textAnchor="middle" className="text-[10px] font-medium" fill="#737373">
              {cp.label}
            </text>
            <text x={rightX + 70} y={cp.y + 4} textAnchor="start" className="text-[9px] font-bold" fill="#0a0a0a">
              {totalForCat}
            </text>
          </g>
        );
...
export default function MatchingPage() {
  ...
  const roomMatches = useMemo(() => computeRoomMatches(), []);
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 whitespace-nowrap border-b-2 px-4 py-3 text-[12px] font-medium transition-colors ${
                  activeTab === tab.id ? "border-foreground text-foreground" : "border-transparent text-muted hover:text-foreground"
                }`}
              >
                <Icon size={14} />
                {tab.label}
          ...
          <div className="space-y-6">
                ...
                {mockBriefs.map((b, i) => (
                  <button
                    ...
                    className={`rounded-xl border p-4 text-left transition-all ${
                      selectedBrief === i ? "border-foreground ring-2 ring-foreground/10 bg-surface/50" : "border-border hover:border-foreground/30"
                    }`}
                  >
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {brief.features.map((f) => <FitBadge key={f} label={f} active />)}
                  </div>
                </div>
                <div>
                  <p className="text-[10px] font-semibold text-muted uppercase tracking-wider mb-1.5">Materials</p>
                  <div className="flex flex-wrap gap-1">
                    {brief.materials.map((m) => <FitBadge key={m} label={m} active />)}
                  </div>
                </div>
                <div>
                  <p className="text-[10px] font-semibold text-muted uppercase tracking-wider mb-1.5">Rooms</p>
                  <div className="flex flex-wrap gap-1">
                    {brief.rooms.map((r) => <span key={r} className="rounded-full bg-surface px-2 py-0.5 text-[9px] font-medium text-muted">{r}</span>)}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-border bg-white overflow-hidden">
              ...
              <div className="divide-y divide-border">
                {briefProductMatches.map((match, i) => (
        ...
        {activeTab === "brief-architect" && (
          <div className="space-y-6">
                  ...
                  {mockBriefs.map((b, i) => (
                    <button
                      ...
                      className={`rounded-lg px-3 py-1.5 text-[11px] font-medium transition-colors ${
                        selectedBrief === i ? "bg-foreground text-white" : "bg-surface text-muted hover:bg-surface-hover"
                      }`}
                    >
                      {b.title}
                    </button>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {briefArchitectMatches.map((match, i) => (
                <div key={match.architect.id} className={`rounded-2xl border bg-white p-5 transition-all ${i < 2 ? "border-foreground ring-1 ring-foreground/5" : "border-border"}`}>
                  <div className="flex items-start gap-4">
                      ...
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {match.architect.specialties.map((s) => (
                          <FitBadge
                            key={s}
                            label={s}
                            active={match.styleOverlap.includes(s)}
                          />
                        ))}
                      </div>
                    </div>
                    <button
                      ...
                      className={`rounded-lg px-3 py-1.5 text-[11px] font-medium transition-colors ${
                        shortlistedArchitects.has(match.architect.id)
                          ? "bg-foreground text-white"
                          : "border border-border text-muted hover:bg-surface"
                      }`}
                    >
                      {shortlistedArchitects.has(match.architect.id) ? (
                        <><Check size={11} className="inline mr-1" />Shortlisted</>
                      ) : (
                        <><Star size={11} className="inline mr-1" />Shortlist</>
                      )}
                    </button>
                  </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {projects.map((proj, i) => (
                  <button
                    ...
                    className={`rounded-xl border p-4 text-left transition-all ${
                      selectedProject === i ? "border-foreground ring-2 ring-foreground/10 bg-surface/50" : "border-border hover:border-foreground/30"
                    }`}
                  >
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="rounded-2xl border border-border bg-white p-5">
                    ...
                    <p className="text-[28px] font-bold">{currentProjectMatch.projectProducts.length}</p>
                    <div className="mt-2 flex flex-wrap gap-1">
                      {[...currentProjectMatch.usedCategories].map((c) => (
                  ...
                  <div className="rounded-2xl border border-border bg-white p-5">
                    ...
                    <p className="text-[28px] font-bold text-amber">{currentProjectMatch.missingCategories.length}</p>
                    <div className="mt-2 flex flex-wrap gap-1">
                      {currentProjectMatch.missingCategories.map((c) => (
                  ...
                  <div className="rounded-2xl border border-border bg-white p-5">
                    ...
                    <p className="text-[28px] font-bold">{currentProjectMatch.recommended.filter((r) => r.score >= 40).length}</p>
                ...
                <div className="rounded-2xl border border-border bg-white overflow-hidden">
                  <div className="p-5 border-b border-border">
                    <h2 className="text-[14px] font-semibold">Recommended Products for {currentProjectMatch.project.title}</h2>
                    <p className="text-[11px] text-muted mt-0.5">Based on missing categories, architect patterns, and momentum signals</p>
                  </div>
                  <div className="divide-y divide-border">
                    {currentProjectMatch.recommended.slice(0, 8).map((rec, i) => (
            ...
            <div className="rounded-2xl border border-border bg-white p-6">
              ...
              <div className="mb-8">
                ...
                <div className="space-y-3">
                  {recommendationLogic.map((signal, i) => {
          ...
          <div className="space-y-6">
                  ...
                  {[...savedProducts].map((pid) => {
                    ...
                    return (
                      <div key={pid} className="flex items-center gap-3 rounded-xl border border-border p-3">
                        ...
                        <button
                          onClick={() => toggleSaveProduct(pid)}
                          className="rounded-lg p-1.5 text-muted hover:text-rose transition-colors"
                        >
                      ...
                      <div key={aid} className="flex items-center gap-4 rounded-xl border border-border p-4">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex gap-1.5">
                            <button className="rounded-lg border border-border px-3 py-1.5 text-[11px] font-medium text-muted hover:bg-surface">
                              View Profile
                            </button>
                            <button
                              onClick={() => toggleShortlistArchitect(aid)}
                              className="rounded-lg p-1.5 text-muted hover:text-rose transition-colors"
                            >
            ...
            <div className="rounded-2xl border border-border bg-white p-6">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  { label: "Create Design Board", sub: "From saved products", icon: Layers, action: "New Board" },
                  { label: "Generate Specification", sub: "From matched products", icon: Download, action: "New Spec" },
                  { label: "Export Match Report", sub: "PDF summary", icon: BarChart3, action: "Export" },
                  { label: "Share with Team", sub: "Invite collaborators", icon: Users, action: "Share" },
                ].map((qa) => (
